...
def get_results(x):
    ...
    print('num of BL: ', num_PA0, 'BL: ', cor_PA0, 'PA: ', num_PA0 - cor_PA0)
    print('num of PA1: ', num_PA1, 'PA1: ', cor_PA1, 'BL: ', num_PA1 - cor_PA1 )
    print('num of PA2: ', num_PA2, 'PA2: ', cor_PA2, 'BL: ', num_PA2 - cor_PA2 )
    print('num of PA3: ', num_PA3, 'PA3: ', cor_PA3, 'BL: ', num_PA3 - cor_PA3 )
    print('num of PA4: ', num_PA4, 'PA4: ', cor_PA4, 'BL: ', num_PA4 - cor_PA4 )
    print('###############################')
    ...
    print('avg acc: ', float((cor_PA1 + cor_PA2 + cor_PA3  + cor_PA4 + 4*cor_PA0)/(num_PA1 + num_PA2 + num_PA3 + num_PA4 + 4*num_PA0)))
...
class RCNN:
    ...
    def build_model(self):
        ...
        W_fc1 = tf.get_variable("W_fc1", [A * B * 256, 2],
                             initializer=tf.contrib.layers.xavier_initializer())
        b_fc1 = tf.Variable(tf.constant(0.1, shape=[2]))
        logits = tf.nn.elu(tf.matmul(h_flat5, W_fc1) + b_fc1, name='model')

        #d_fc1 = tf.nn.dropout(h_fc1, keep_prob=keep_prob)
        '''
        W_fc2 = tf.get_variable("W_fc2", [64, 2],
                             initializer=tf.contrib.layers.xavier_initializer())
        b_fc2 = tf.Variable(tf.constant(0.1, shape=[2]))
        h_fc2 =  tf.nn.elu(tf.matmul(d_fc1, W_fc2) + b_fc2, name='model')
        logits = tf.nn.dropout(h_fc2 ,keep_prob=keep_prob)
        '''
        global_step = tf.Variable(0, trainable=False, name='global_step')

        bound1 = int(3 * int(len(labels_train)/train_batch_size))
        bound2 = bound1*2
        ...
        ## Loss, Optimizer
        classes_weights = 4
        #cost = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=logits, targets=Y_one_hot, pos_weight=classes_weights), name='cost')
            ...
            for i in range(epoch):
                ...
                for j in range(num_batch):
                    ...
                    train_summary, _, train_cost_val = sess.run([merged, optimizer, cost],
                                                                feed_dict={X: train_batch_xs, Y: train_batch_bin_ys, keep_prob:0.6})
                    ...
                    if (j % 10 == 0):
                        ...
                        valid_summary, valid_cost_val = sess.run([merged, cost],
                                                                 feed_dict={X: valid_batch_xs, Y: valid_batch_bin_ys, keep_prob:1})
                ...
                valid_avg_cost = float(valid_total_cost / int(num_batch / 10))
    ...
    def test_model(self, model_name):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(num_batch):
                ...
                print('Label: ', sess.run(tf.argmax(Y_one_hot, 1), feed_dict={Y: test_batch_bin_ys}))
                #print(test_batch_ys)
                ...
                num_correct = acc.eval(feed_dict={X: test_batch_xs, Y: test_batch_bin_ys, keep_prob:1})
